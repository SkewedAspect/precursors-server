...
proplist(Reps) ->
    benchmark(Reps,
			  [],
			  fun (Key, Value, List) -> lists:keystore(Key, 1, List, {Key, Value}) end,
              fun proplists:get_value/2,
			  fun (Key, Value, List) -> lists:keyreplace(Key, 1, List, {Key, Value}) end,
