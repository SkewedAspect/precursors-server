...
%% @doc Start an application and all of it's required dependencies. This simply
%% tries to start the application, and if it fails, it catches the error and if
%% the error is an unstarted dependency, it attempts to start that application.
start_app(App) ->
	case application:start(App) of
		...
		{error, {not_started, Dependency}} ->
			?debug("Unstarted Dependency ~p detected, starting...", [Dependency]),
