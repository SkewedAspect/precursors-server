...
pre_channel_ping_test_()-> [
	...
	{"Register Hooks Test", fun()->
			meck:new(pre_hooks),
			meck:expect(pre_hooks, add_hook, fun(client_logged_in, pre_channel_ping, client_login_hook, undefined, Nodes)->
						TestNodes = [node()],
	...
	{"Client Login Hook Trigger Test", fun()->
		meck:new(pre_client_channels),
		TestRecord = #client_info{channel_manager = <<"TestString">>},
		meck:expect(pre_client_channels, set_channel, fun(<<"TestString">>, <<"ping">>, pre_channel_ping, [])->
				ok
		end),
	...
	{"Client Request/5 Test", fun()->
		meck:new(pre_client_connection),
		meck:expect(pre_client_connection, send, fun(Connection, tcp, {response, Id}, <<"ping">>, PingResponse)->
			{struct, Items} = PingResponse,
			{Mega, Secs, _} = now(),
			TestTimeStamp = Mega * 1000000 + Secs,
			InTimeStamp = proplists:get_value(timestamp, Items),
