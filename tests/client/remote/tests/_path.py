...
def setPath(filename):
    ...
    # Assuming sys.argv[0] is one of the test modules, up(mainModule) is the `tests` directory, up(up(mainModule)) is the
    # `remote` package's directory, and up(up(up(mainModule))) is the directory containing `remote`.
